...
export async function PATCH(request: NextRequest) {
  try {
    const supabase = createClient();
    const { livre_id, isbn } = await request.json(); // On attend { livre_id, isbn }

    // Mettre à jour le champ ISBN pour le livre ciblé
    const { data, error } = await supabase
      .from('inventaire')
      .update({ isbn })
      .eq('livre_id', livre_id) // ou .eq('livre_id', id) si ton champ s'appelle livre_id
      .select()
      .maybeSingle();
    ...
    return new Response(
      JSON.stringify({ message: 'ISBN mis à jour', produit: data, success: true }),
      { status: 200 }
    );
  } catch (err: any) {
    return new Response(
